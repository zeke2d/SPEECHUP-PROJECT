...
app.get("/setappointment", async (req, res) => {
    if (!req.session.user) {
        return res.redirect("/patientlogin"); // Redirect if not logged in
    }

    try {
        const therapists = await Therapist.find({}, 'name email workingHours'); // Fetch all therapists
        res.json(therapists);
      } catch (error) {
...
app.get("/content/setappointment", async (req, res) => {
     if (!req.session.user) {
      return res.status(401).send("Unauthorized");
    }
